...
## Phase 2B1 — player letter collection and word validation test.
...
const RAIN_PROFILE := preload("res://resources/letters/rain_spawn_profile.tres")
...
func _ready() -> void:
	...
	letter_spawner.profile = RAIN_PROFILE
	...
	WordGameFeatures.attach_profanity_reactions($UI, word_controller)
