class_name ClawCollectibleSpawner
extends Node2D

## Occasionally drops a single CLAW pickup from above the playfield.
...
func _ready() -> void:
	_scene = load("res://scenes/collectibles/claw_collectible.tscn") as PackedScene
...
func _active_count() -> int:
	var count := 0
	for node in get_tree().get_nodes_in_group("claw_collectible"):
...
func _spawn_one() -> void:
	if _scene == null:
		return
	var pickup: ClawCollectible = _scene.instantiate()
	add_child(pickup)
